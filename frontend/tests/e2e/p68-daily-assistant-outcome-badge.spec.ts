/**
 * P68 — Daily Assistant Outcome Known Badge
 *
 * Validates that DailyAssistantEntry renders an explicit positive badge in the
 * 7-day outcome section when fbSummary.improved_count > 0:
 *
 * 1. [data-testid="daily-summary-outcome-improved-badge"] visible when improved_count > 0
 * 2. Badge absent when improved_count = 0
 * 3. P67 regression: daily-summary-action-impact still visible when todayAction populated
 * 4. P66 regression: daily-summary-missing-data-explanation still visible with missing data
 * 5. P65 regression: daily-summary-why-now still visible when whyNow populated
 * 6. Dashboard renders without ErrorBoundary fallback
 *
...
const BASE_DASHBOARD = {
  ...
  recommendations: [],
  trends: {},
  explainability_summary: 'P68 mock summary',
...
/** Daily summary with all fields populated */
...
/** Outcome feedback with improved_count > 0 */
...
test.describe('P68 — Daily Assistant Outcome Known Badge', () => {

  test('improved badge visible when improved_count > 0', async ({ page }) => {
  ...
  test('improved badge absent when improved_count = 0', async ({ page }) => {
    await setAuthStorage(page)
    await stubRoutes(page, {
      dailySummary: DAILY_SUMMARY_FULL,
      outcomeFeedback: OUTCOME_NO_IMPROVED,
    })

    await page.goto('/platform/dashboard')
    await expect(page.locator('[data-testid="daily-assistant-entry"]')).toBeVisible({ timeout: 12_000 })

    // Outcome section still renders (total_count = 3)
    await expect(page.locator('[data-testid="daily-summary-outcome-section"]')).toBeVisible()

    // But badge must NOT appear
