...
const BASE_DASHBOARD = {
  ...
  recommendations: [],
  trends: {},
  explainability_summary: 'P126 mock',
...
async function setAuthStorage(page: import('@playwright/test').Page) {
  await page.addInitScript(() => {
    localStorage.setItem('token', 'p126-token')
...
test.describe('P126 — First-Run Activation Polish Contract', () => {
  test('1) empty state shows explicit next-step CTAs without ErrorBoundary', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="first-run-empty-cta-documents"]')).toBeVisible()
    await expect(page.locator('[data-testid="first-run-empty-cta-symptoms"]')).toBeVisible()
    await expect(page.getByText('載入失敗，請重新整理')).not.toBeVisible()
  })

  test('2) report-only shows next-step symptom guidance with reason copy', async ({ page }) => {
    await setAuthStorage(page)
    ...
    await expect(page.locator('[data-testid="first-run-why-symptoms"]')).toBeVisible()
    await expect(page.locator('[data-testid="first-run-step-documents-status"]')).toContainText('已完成')
    await expect(page.locator('[data-testid="first-run-step-symptoms-status"]')).toContainText('尚未開始')
  })

  test('3) symptom-only shows next-step documents guidance with reason copy', async ({ page }) => {
    await setAuthStorage(page)
    await stubRoutes(page, {
      documents: [],
    ...
    await expect(page.locator('[data-testid="first-run-why-documents"]')).toBeVisible()
    await expect(page.locator('[data-testid="first-run-step-documents-status"]')).toContainText('尚未開始')
    await expect(page.locator('[data-testid="first-run-step-symptoms-status"]')).toContainText('已完成')
  })

  test('4) completed state shows CTA to dashboard and actions', async ({ page }) => {
    await setAuthStorage(page)
    await stubRoutes(page, {
      documents: [{ id: 'd1', parse_status: 'confirmed' }],
      ...
      recommendations: {
        person_id: 'person-self',
        recommendations: [
          makeRec({ is_tracking: true, source_type: 'lab_report_item', source_id: 'lab-1', evidence_summary: '健檢報告：LDL 偏高' }),
        ],
        total: 1,
        missing_data: [],
      },
    ...
    await expect(page.locator('[data-testid="first-run-completed-cta-dashboard"]')).toBeVisible()
    await expect(page.locator('[data-testid="first-run-completed-cta-actions"]')).toBeVisible()
    await expect(page.locator('[data-testid="first-run-step-actions-status"]')).toContainText('已開始追蹤')
  })

  test('5) CTA links point to existing routes only', async ({ page }) => {
    await setAuthStorage(page)
    await stubRoutes(page, { documents: [], symptoms: [], dailySummary: DAILY_SUMMARY_EMPTY })
    await page.goto('/platform/dashboard')
    ...
    await expect(page.locator('[data-testid="first-run-empty-cta-documents"]')).toHaveAttribute('href', '/platform/documents')
    await expect(page.locator('[data-testid="first-run-empty-cta-symptoms"]')).toHaveAttribute('href', '/platform/symptoms')
  })

  test('6) suppressed/not-judged evidence never described as normal', async ({ page }) => {
    await setAuthStorage(page)
    await stubRoutes(page, {
      documents: [{ id: 'd1', parse_status: 'confirmed' }],
      symptoms: [{ id: 's1', symptom: '疲勞', occurred_at: new Date().toISOString() }],
  ...
  test('7) overclaim guard: prohibited medical claim phrases are absent', async ({ page }) => {
    await setAuthStorage(page)
    await stubRoutes(page, {
      documents: [{ id: 'd1', parse_status: 'confirmed' }],
      symptoms: [{ id: 's1', symptom: '疲勞', occurred_at: new Date().toISOString() }],
